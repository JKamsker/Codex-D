...
public sealed class ThreadArchiveCommand : AsyncCommand<ThreadArchiveCommand.Settings>
{
    ...
    public override async Task<int> ExecuteAsync(CommandContext context, Settings settings, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var response = await client.ArchiveThreadAsync(settings.ThreadId, cancellationToken);
            ...
            else
            {
                ...
                AnsiConsole.MarkupLine($"Archived: [cyan]{Markup.Escape(id)}[/]");
            }
        }
        catch (Exception ex)
        {
            if (format != OutputFormat.Human)
            {
                CliOutput.WriteJsonError("thread_archive_failed", ex.Message ?? string.Empty);
            }
            else
            {
                AnsiConsole.MarkupLine($"[red]Failed to archive thread:[/] {Markup.Escape(ex.Message ?? string.Empty)}");
            }
            return 1;
        }
